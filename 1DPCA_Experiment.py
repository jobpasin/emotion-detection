...
#-------------------Initialize ----------------------------------
Path = 1
Use_testpic = 1                     #0 = use remaining train picture as test picture
InverseOrder = 0                    #1 = inverse order of picture(will use glasses picture first
if Path == 1:
    path_Train = './TrainingPics'   #Training picture path
    path_Test = './TestingPics'     #Testing picture path
    Data_path ='./Report/Data1D_NoGlass.txt'        #Text data saving path ####
elif Path == 2:
    path_Train = './NitadDB/TrainingA'        #Training picture path
    path_Test = './NitadDB/TestingA'     #Testing picture path
    Data_path ='./Report/1D_Nitad_A_A.txt'        #Text data saving path ####
else:
     print 'Path Error'   

num_TrainPic = 16               #Max number of training pic
num_Eig_Max = 11                 #Max number of Eig
num_start_max = 5               #Ignore some highest eigenvectors
...
def import_images(path, emotion,num_Pic,Use_testpic,path_Test): #Process of importing images from a folder
    ...
    for image_path in image_paths:
        ...
        if np.max(im_av) >= 65000: #float16 overflow at 65504
            print "Warning, data may overflow, change to float32"
        pic_cnt = pic_cnt + 1
        if (pic_cnt == num_Pic):
            break
    if Use_testpic == 0:
    ...
    print "Test image:",image_paths
    return image_vec, image_paths,im_av
...
def create_Eig(images,im_av,num_Eig,num_Ignore): #Process of creating optimal projection axis
    ...
    return Eig,EgVal[num_Ignore:num_Ignore+num_Eig]
...
File.write("Error%Happy/Correction%Sad/#PCA%Angry/#Ignore%Surprised%Real Emotion%Detect as%# PCA%Ignore%Num of Training picture\n") 
#print "Number of optimal projection axis is:", num_optproj
#----------------Train Code -----------------------------#
for num_Pic in range(3,num_TrainPic+1):
#for num_Pic in range(num_TrainPic,num_TrainPic+1):
    # Importing image
    train_images = range(4)
    test_Images_Path = range(4)
    Im_Av = range(4)
    train_images[0],test_Images_Path[0],Im_Av[0] = import_images(path_Train,Happy,num_Pic,Use_testpic,path_Test)
    train_images[1],test_Images_Path[1],Im_Av[1] = import_images(path_Train,Sad,num_Pic,Use_testpic,path_Test)
    train_images[2],test_Images_Path[2],Im_Av[2] = import_images(path_Train,Angry,num_Pic,Use_testpic,path_Test) 
    train_images[3],test_Images_Path[3],Im_Av[3] = import_images(path_Train,Surprised,num_Pic,Use_testpic,path_Test)
    for num_Ignore in range(0,num_start_max+1):
        ...
        #for num_Eig in range (1,2):
            ...
            for cnt in range(0,4):
                Eig[cnt],EigVal = create_Eig(train_images[cnt],Im_Av[cnt],num_Eig,num_Ignore)                                   
                if min(EigVal) < 100:
                    raise ValueError('WARNING, TOO LOW EIGENVALUE')
            ...
            #File.write("Num of training picture: %"+str(num_Pic)+"%# of PCA: %"+str(num_Eig)+"% Ignore: %"+str(num_Ignore)+"\n")
            ...
            for emotion_type in range(0,4):
                ...
                #image_paths = [os.path.join(path_Test, f) for f in os.listdir(path_Test) \
                #               if f.split(".")[0].endswith(emotion)]
                print "Number of testing image ", emotion, "is:", len(test_Images_Path[emotion_type]), "pictures"
                print "num_Pic",num_Pic,"num_Eig",num_Eig,"num_Ignore",num_Ignore
                for image_path in test_Images_Path[emotion_type]:
                    #Import testing image
                    Im_Test = import_image(image_path)
                    ...
                    MinEmotion = 0                                             
                    for cnt in range(0,4):                                         
                        Error_Temp = TestImage(Im_Test,Eig[cnt],Im_Av[cnt])
                ...
                File.write("Correction for "+emotion+" is%" + str(correct_pic_temp/float(total_pic_temp)*100)+"%"+str(num_Eig)+"%"+str(num_Ignore)+"%"+str(num_Pic)+" %\n")
                File2.write("Correction for "+emotion+" is%" + str(correct_pic_temp/float(total_pic_temp)*100)+" %\n")
            print "Total accuracy is: ",str(correct_pic/float(total_pic)*100), "%\n"                                             
            File.write("Total accuracy is%" + str(correct_pic/float(total_pic)*100)+"%"+str(num_Eig)+"%"+str(num_Ignore)+"%"+str(num_Pic)+" %\n")
            File2.write("   Total accuracy is%" + str(correct_pic/float(total_pic)*100)+" %\n")   
                    ##if MinEmotion == 0: 
                    ##    Emotion = 'Happy'
                    ##elif MinEmotion == 1:
                    ##    Emotion = 'Sad'                                             
...
print "1DPCA Experiment Finished"
if InverseOrder == 1:
